...
class ResourceManager(object):
    ...
    def __init__(self, basedir="../assets"):
        self.__base_dir = path.abspath(path.join(path.dirname(__file__), basedir))
        ...
        for root, dirs, files in walk(self.__base_dir):
            for file_ in files:
                if file_.split(".")[-1] == "png":
                    ...
                    if type_ == "icon":
                        self.__resources[type_][name] = wx.Icon(path.join(root, file_), wx.BITMAP_TYPE_PNG)
                    elif type_ == "entity" or type_ == "tool":
                        self.__resources[type_][name] = pygame.image.load(path.join(root, file_))
                    else:
                        self.__resources[type_][name] = wx.Bitmap(path.join(root, file_), wx.BITMAP_TYPE_PNG)
